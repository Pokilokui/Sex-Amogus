...
void CFuncTank::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	else if ( m_hController != pPlayer && useType != USE_OFF )
	{
		...
		DevMsg("le caca magique\n");

		//if (pPlayer->GetKeyValue("+mouse2"))
...
void CFuncTank::Think( void )
{
	...
	if (GetController() && GetController()->IsPlayer())
	{
		...
		{
			StopControl();
			const char* pTankName = "turret_2_model";
			if (pTankName && pTankName[0])
			{
				CBaseEntity* pEnt = NULL;

				// 3. Boucle : Chercher TOUTES les entités qui ont ce nom
				// gEntList.FindEntityByName parcourt tout le monde
				while ((pEnt = gEntList.FindEntityByName(pEnt, pTankName)) != NULL)
				{
					// Sécurité : On ne se supprime pas soi-même tout de suite
					// pour ne pas casser la boucle ou crasher le jeu
					if (pEnt != this)
					{
						UTIL_Remove(pEnt);
					}
