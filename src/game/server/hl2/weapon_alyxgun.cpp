...
bool CWeaponAlyxGun::Reload(void)		// Fix reload sound
{
	...
	{
		if (DefaultReload(GetMaxClip1(), GetMaxClip2(), ACT_VM_SMGRELOAD))
			WeaponSound(RELOAD);
	}
...
{
	if (fireMode)
		return ACT_VM_SMGDRAW;
	else
		return ACT_VM_DRAW;
}

Activity CWeaponAlyxGun::GetPrimarryAttackActivity(void)
{
	if (fireMode)
	{
		if (m_nShotsFired < 2)
			return ACT_VM_SMGFIRE;

		if (m_nShotsFired < 3)
			return ACT_VM_SMGFIRE1;

		if (m_nShotsFired < 4)
			return ACT_VM_SMGFIRE2;
		
		return ACT_VM_SMGFIRE3;
