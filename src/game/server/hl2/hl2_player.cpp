...
static ConVar sv_weapon_drop_enabled("sv_weapon_drop_enabled", "1");	// Weapon drop system
...
CHL2_Player::CHL2_Player()
{
	...
	//kick
	m_bInKickTime = false;
	m_fNextKickTime = 0.0f;
	m_fNextSwingTime = 0.0f;
	//
}
...
//ConCommand setarmsmodel("setarmsmodel", ArmsModel, "", 0);
...
void CHL2_Player::Precache( void )
{
	...
	PrecacheModel("models/playermodels/gordon.mdl");
	PrecacheModel("models/playermodels/male_07.mdl");
	PrecacheModel("models/playermodels/police.mdl");

	//c_arms

	PrecacheModel("models/weapons/c_arms_base.mdl");
...
void CHL2_Player::EquipSuit( bool bPlayEffects )
{
	...
	//kick
	//set the kick model to the suit kick model
	//CBaseViewModel* vm_kick = FindOrCreateKick(); ouste
	//vm_kick->SetWeaponModel(KICK_MODEL_SUIT, nullptr);
	//kick
...
//					Warning( "In train mode with no train!\n" );
	...
	if (m_afButtonPressed & IN_DROP)			// Weapon drop system.
	{
		if (IsAlive() &&
			!IsInAVehicle() &&
			HasWeapons() &&
...
//kick
ConVar hl2_kick_distance("hl2_kick_distance", "90");
ConVar hl2_kick_force("hl2_kick_force", "900");
ConVar hl2_kick_damage("hl2_kick_damage", "20");
//
...
void CHL2_Player::PostThink(void)
{
	...
	//kick
	//check if we should stop the kick animation
	if (gpGlobals->curtime >= m_fNextSwingTime)
	{
		//set the kick animation to the idle animation
		//KickSetAnimation(ACT_VM_IDLE);
		//return;
	}

	//update the kick animation 
	//CBaseViewModel* vm_kick = FindOrCreateKick();
	//vm_kick->StudioFrameAdvance();

	if (m_bInKickTime && gpGlobals->curtime >= m_fNextKickTime)
	{

		//only kick once
		//m_bInKickTime = false;

		//do a cool view punch
		ViewPunch(QAngle(random->RandomFloat(-5, 6), random->RandomFloat(-5, 5), random->RandomFloat(-2, 2)));

		//get the eye direction + angles
		QAngle EyeDir = EyeAngles();
		Vector VecDir;
		AngleVectors(EyeDir, &VecDir);

		//get start pos and end pos
		Vector VecStart = EyePosition();
		Vector VecEnd = VecStart + (VecDir * hl2_kick_distance.GetFloat());

		//do some math to make the z position of the kick be lower
		if (EyeDir.x < 30 && EyeDir.x > -30)
		{
			float value = cos(EyeDir.x) * 25;
			if (value < 0) value *= -1;	//value can not be negative
			VecEnd.z = VecEnd.z - value;
		}

		//do the trace line
		trace_t trace;
		UTIL_TraceLine(VecStart, VecEnd, MASK_ALL, this, COLLISION_GROUP_NONE, &trace);

		//get the entity
		CBaseEntity* ent = trace.m_pEnt;
		if (!ent || (ent && ent->IsWorld()))
		{
			if (trace.DidHitWorld())
			{
				//leave a decal
				ImpactTrace(&trace, DMG_GENERIC);
				EmitSound("HL2Player.KickHitWall");
			}
			else
				EmitSound("HL2Player.KickNotHit");

			return;
		}

		if (ent->GetBaseAnimating() && ent->GetBaseAnimating()->IsDissolving())
		{
			//no hit dissolving ents
			EmitSound("PortalPlayer.KickNotHit");
			return;
		}

		EmitSound("PortalPlayer.KickNotHit");	//i know the name is confusing but it will work

		//get the physics object
		IPhysicsObject* ipo = ent->VPhysicsGetObject();

		//deal damage to npcs because why not
		CAI_BaseNPC* npc = dynamic_cast<CAI_BaseNPC*>(ent);

		if (npc && !npc->IsPlayerAlly())
		{
			//
			DoImpactEffect(trace, DMG_GENERIC);

			//take damage to this entity
			CTakeDamageInfo info;
			info.SetAttacker(this);
			info.SetDamage(hl2_kick_damage.GetInt());

			npc->TakeDamage(info);

			if (npc->ClassMatches("npc_csanner") || npc->ClassMatches("npc_clawscanner"))
				return;

			Vector force = VecDir * (hl2_kick_force.GetFloat() / 10);
			if (!npc->IsAlive())
			{
				//if the npc isnt alive then fling its body more
				force = VecDir * hl2_kick_force.GetFloat();
			}

			if (ipo)
				ipo->AddVelocity(&force, nullptr);
			return;
		}

		//add velocity to the entity
		if (ipo && !dynamic_cast<CPropVehicle*>(ent))
		{
			Vector vel = VecDir * (hl2_kick_force.GetFloat());
			ipo->AddVelocity(&vel, nullptr);
		}

		//the impact effect makes some entity's crash. so do this
		if (ent->ClassMatches("prop_physics") || ent->ClassMatches("prop_combine_ball"))
			return;
...
void CHL2_Player::Spawn(void)
{
	...
	ApplyCustomHands();
	//FindOrCreateKick();		//kick
}

void CHL2_Player::ApplyCustomHands()
{
	CBaseViewModel* pHandsVM = GetViewModel(1); // 1 = arms
	if (pHandsVM && GetActiveWeapon())
	{
		pHandsVM->SetWeaponModel(cl_armsmodel.GetString(), GetActiveWeapon());
	}
}
...
//-----------------------------------------------------------------------------
// Purpose: creates the kick viewmodel
//-----------------------------------------------------------------------------
/*CBaseViewModel* CHL2_Player::FindOrCreateKick()
{
	//find the viewmodel
	CBaseViewModel* vm_kick = GetViewModel(VIEWMODEL_INDEX_KICK);
	if (vm_kick)
		return vm_kick;

	//create the viewmdel because it wasnt found
	CreateViewModel(VIEWMODEL_INDEX_KICK);
	vm_kick = GetViewModel(VIEWMODEL_INDEX_KICK);

	//if the player has the suit then set the model to the suit model. else vice versa
	vm_kick->SetWeaponModel(IsSuitEquipped() ? KICK_MODEL_SUIT : KICK_MODEL_NO_SUIT, nullptr);

	return vm_kick;
}

//-----------------------------------------------------------------------------
// Purpose: Sets the kick viewmodel's animation then return how long the sequence last's for
//-----------------------------------------------------------------------------
float CHL2_Player::KickSetAnimation(Activity act)
{
	CBaseViewModel* vm_kick = GetViewModel(VIEWMODEL_INDEX_KICK);
	if (!vm_kick)
	{
		//fail safe
		vm_kick = FindOrCreateKick();
	}

	//get the animation index
	float AnimIndex = vm_kick->SelectWeightedSequence(act);
	if (AnimIndex >= 0)
	{
		vm_kick->SendViewModelMatchingSequence(AnimIndex);
		return vm_kick->SequenceDuration(AnimIndex); //return the animation duration
	}
...
bool CHL2_Player::ClientCommand( const CCommand &args )
{
	...
	//kick
	/*else if (!Q_strcmp(args[0], "player_kick"))
	{
		if (IsInAVehicle())
			return true;

		m_bInKickTime = true;
		m_fNextSwingTime = gpGlobals->curtime + KickSetAnimation(ACT_VM_PRIMARYATTACK) - 0.4;
		m_fNextKickTime = gpGlobals->curtime + 0.12;
...
void CHL2_Player::DropActiveWeapon(void)			// Weapon drop system.
{
	Vector VecForward;
