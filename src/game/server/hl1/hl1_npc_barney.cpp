...
ConVar	sk_hl1_barney_health( "sk_hl1_barney_health","35");
...
void CNPC_HL1Barney::Spawn()
{
	...
	SetModel( "models/hl1/barney.mdl");
	...
	m_iHealth			= sk_hl1_barney_health.GetFloat();
...
void CNPC_HL1Barney::Precache()
{
	...
	//m_iAmmoType = GetAmmoDef()->Index("9mm_bullet");

	PrecacheModel("models/hl1/barney.mdl");
...
//	SetBlending( 0, angDir.x );
	...
	data.m_flScale = 1.0f;
	data.m_fFlags = MUZZLEFLASH_PISTOL;
...
void CNPC_HL1Barney::PainSound( const CTakeDamageInfo &info )
{
	...
	m_flPainTime = gpGlobals->curtime + random->RandomFloat(5.0, 75.0);
...
void CNPC_HL1Barney::Event_Killed( const CTakeDamageInfo &info )
{
	if ( m_nBody < BARNEY_BODY_GUNGONE )
	{
		...
		angGunAngles.y += 180;
		pGun = DropItem( "weapon_hl1_glock", vecGunPos, angGunAngles );
	}
