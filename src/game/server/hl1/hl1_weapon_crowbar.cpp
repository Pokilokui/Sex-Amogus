...
extern ConVar sk_hl1_plr_dmg_crowbar;
...
#ifdef CLIENT_DLL
#define CWeaponHL1Crowbar C_WeaponCrowbar
#endif
...
LINK_ENTITY_TO_CLASS( weapon_hl1_crowbar, CWeaponHL1Crowbar );
PRECACHE_WEAPON_REGISTER( weapon_hl1_crowbar );
...
acttable_t CWeaponHL1Crowbar::m_acttable[] =
{
...
IMPLEMENT_ACTTABLE(CWeaponHL1Crowbar);
...
void CWeaponHL1Crowbar::Hit( void )
{
	...
	if ( pHitEntity != NULL )
	{
		...
		ClearMultiDamage();
		CTakeDamageInfo info( GetOwner(), GetOwner(), sk_hl1_plr_dmg_crowbar.GetFloat(), DMG_CLUB );
		...
		TraceAttackToTriggers( CTakeDamageInfo( GetOwner(), GetOwner(), sk_hl1_plr_dmg_crowbar.GetFloat(), DMG_CLUB ), m_traceHit.startpos, m_traceHit.endpos, hitDirection );
