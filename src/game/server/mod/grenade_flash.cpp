...
#include "soundent.h"
#include "engine/IEngineSound.h"		//Flash grenade
...
#endif
	void Detonate(void);		//Flash grenade
...
#define CONC_FALLOFF_TIME 5.0f		//Flash grenade

void CGrenadeFlash::Detonate(void)		//Flash grenade
{
	// An array for all the player pointers to chill
	CBaseEntity* list[1024];

	Vector start = GetAbsOrigin() + Vector(0, 10, 0);

	// grab all of them within 400 units
	int count = UTIL_EntitiesInSphere(list, 1024, start, 400, MASK_PLAYERSOLID);
	...
	for (int i = 0; i < count; i++)
	{
		...
		if (list[i]->IsPlayer())
		{
			...
			float distancesq = (pPlayer->GetAbsOrigin() - start).LengthSqr();
			if (distancesq > 16000)
				break;
			...
			if (distancesq >= 40000)
				ratio = 1.0f - pow((FastSqrt(distancesq) / 200.0) - 1, 2);
...
bool CGrenadeFlash::HandleInteraction(int interactionType, void* data, CBaseCombatCharacter* sourceEnt)
{
...
void CGrenadeFlash::InputSetTimer(inputdata_t& inputdata)
{
...
bool Flashgrenade_WasPunted(const CBaseEntity* pEntity)
{
	const CGrenadeFlash* pFrag = dynamic_cast<const CGrenadeFlash*>(pEntity);
	if (pFrag)
	{
...
bool Flashgrenade_WasCreatedByCombine(const CBaseEntity* pEntity)
{
	const CGrenadeFlash* pFrag = dynamic_cast<const CGrenadeFlash*>(pEntity);
	if (pFrag)
	{
