...
class CWeaponHL1357 : public CBaseCombatWeapon
{
	DECLARE_CLASS( CWeaponHL1357, CBaseCombatWeapon);
public:
...
acttable_t CWeaponHL1357::m_acttable[] =
{
...
IMPLEMENT_ACTTABLE(CWeaponHL1357);

IMPLEMENT_NETWORKCLASS_ALIASED( WeaponHL1357, DT_WeaponHL1357 );

BEGIN_NETWORK_TABLE(CWeaponHL1357, DT_WeaponHL1357)
#ifdef CLIENT_DLL
...
BEGIN_PREDICTION_DATA( CWeaponHL1357 )
#ifdef CLIENT_DLL
	DEFINE_PRED_FIELD( m_fInZoom, FIELD_FLOAT, FTYPEDESC_INSENDTABLE ),
#endif
END_PREDICTION_DATA()

LINK_ENTITY_TO_CLASS( weapon_hl1_357, CWeaponHL1357 );

PRECACHE_WEAPON_REGISTER( weapon_hl1_357 );
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CWeaponHL1357::CWeaponHL1357( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponHL1357::Precache( void )
{
	BaseClass::Precache();
...
bool CWeaponHL1357::Deploy( void )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponHL1357::PrimaryAttack( void )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponHL1357::SecondaryAttack( void )
{
	// only in multiplayer
	if ( !g_pGameRules->IsMultiplayer() )
	{
...
bool CWeaponHL1357::Reload( void )
{
	bool fRet;
...
void CWeaponHL1357::WeaponIdle( void )
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
	if ( pPlayer )
	{
...
bool CWeaponHL1357::Holster( CBaseCombatWeapon *pSwitchingTo )
{
	if ( m_fInZoom )
	{
		ToggleZoom();
...
void CWeaponHL1357::ToggleZoom( void )
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
	if ( !pPlayer )
	{
