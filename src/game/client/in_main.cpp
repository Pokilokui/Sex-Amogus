...
kbutton_t	in_drop;			// Weapon drop system.
...
void IN_DropUp(const CCommand& args) { KeyUp(&in_drop, args[1]); }					// Weapon drop system.
void IN_DropDown(const CCommand& args) { KeyDown(&in_drop, args[1]); }				// Weapon drop system.
...
int CInput::GetButtonBits( int bResetState )
{
	...
	CalcButtonBits(bits, IN_DROP, s_ClearInputState, &in_drop, bResetState);					// Weapon drop system.	
...
static ConCommand enddrop("-drop", IN_DropUp);							// Weapon drop system.
static ConCommand startdrop("+drop", IN_DropDown);						// Weapon drop system.
