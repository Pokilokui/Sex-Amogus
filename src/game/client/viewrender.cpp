...
static inline void DrawOpaqueRenderable( IClientRenderable *pEnt, bool bTwoPass, ERenderDepthMode DepthMode, int nDefaultFlags = 0 )
{
	...
	//C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();			//Decals + flashlight on models
	...
	//Decals + flashlight on models
	/*if (pPlayer && pPlayer->IsEffectActive(EF_DIMLIGHT) &&
		(pPlayer->GetAbsOrigin() - ((C_BaseEntity*)pEnt)->GetAbsOrigin()).Length()
		< r_flashlightfar->GetFloat() &&
		(
			(dynamic_cast<C_BaseEntity*>(pEnt) //&& ((C_BaseEntity*)pEnt)->IsNPC() || dynamic_cast<C_ClientRagdoll*>(pEnt))
		)))
	{
